...
const SAMPLE_SIGNATURE = "3045022100a4e7f8d6... (Select load sample to populate)";
...
export default function VerifyPage() {
  ...
  const handleLoadSample = async () => {
    setReportText(JSON.stringify(SAMPLE_PAYLOAD, null, 2));
    setSignature('');
    setVerificationResult('idle');
    setErrorMsg('');
    
    // We fetch a real signature from the backend if possible, or mock one for demonstration
    // Since there's no static demo key, the easiest way to get a perfectly valid signature
    // in a new database is for the backend to generate it, but since we are a standalone tool,
    // we can guide the user to perform a scan on the detail page to get a real signature,
    // OR we can fetch one from a temporary scan. Let's make it easy: we prompt them.
    // Wait, let's look at the contract. We can fetch the public key.
    // For a real signature, they should run a scan first. Let's write instructions for them.
    setSignature("6a117b4c9e830e2f5d762b71... (Paste a real signature from your repository scans below)");
  };
              ...
              {verificationResult === 'valid' && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400 px-4">
                    The report matches the cryptographic signature and was verified using DarkShield's private key. The payload has not been tampered with.
                  </p>
                </div>
              )}
